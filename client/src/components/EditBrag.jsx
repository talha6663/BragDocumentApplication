...
const EditBrag = ({item, refreshList}) => {
    ...
    return (
        <>
			<FaEdit className="mt-[2px] w-3 h-3 text-slate-50 group-hover:text-slate-600 dark:text-zinc-950 dark:group-hover:text-slate-400 ml-3 cursor-pointer" title="Edit Brag" onClick={openModal} />
            ...
            <dialog ref={modalRef} className="p-4 w-[500px] bg-slate-50 border-slate-500 dark:bg-zinc-950 dark:border-zinc-800 rounded-md border-4">
                ...
                <textarea className="mb-3 p-3 w-full bg-slate-200 border-2 rounded-md border-slate-300 dark:bg-slate-800 dark:border-slate-800 dark:text-slate-200 focus:outline-none" rows="10" value={brag} onChange={(e) => setBrag(e.target.value)} />
