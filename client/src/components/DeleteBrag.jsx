...
const DeleteBrag = ({ item, refreshList }) => {
  ...
  const deleteBrag = async () => {
    if (!item || !item._id) {
      console.error("Item or _id is undefined");
      return;
    }

    try {
      const response = await fetch(`http://localhost:5000/brags/${item._id}`, {
        method: 'DELETE',
      });

      if (!response.ok) {
        throw new Error('Error deleting brag');
      }

      refreshList();
    } catch (err) {
      console.error('Error deleting brag:', err.message);
    }
  };
  ...
  return (
    <>
      <FaTrash
        className="mt-[2px] w-3 h-3 lg:text-slate-50 group-hover:text-slate-600 lg:dark:text-midnight-900 dark:group-hover:text-midnight-100 ml-1 lg:ml-3 cursor-pointer"
        title="Delete Brag"
        onClick={openModal}
      />
      ...
      <dialog ref={modalRef} className="p-4 w-[500px] bg-slate-50 border-slate-500 dark:bg-midnight-900 dark:border-midnight-800 rounded-md border-4">
        <p className="text-center text-xl font-bold text-neutral-600 dark:text-neutral-300 mb-2">
          Delete the following brag?
        </p>

        <p className="text-center truncate mb-2 p-3 border-2 rounded-md border-neutral-300 text-neutral-600 dark:bg-midnight-700 dark:border-midnight-700 dark:text-neutral-200">
          {item.brag}
        </p>

        <p className="text-center font-bold text-red-600 dark:text-red-500">
